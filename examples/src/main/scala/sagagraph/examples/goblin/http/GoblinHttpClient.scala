...
import sagagraph.examples.goblin.{
  Goblin,
  Weapon,
  Uniform,
  Boot,
...
object GoblinHttpClient:
  ...
  def measure(name: String): Either[Throwable, Goblin] =
    try
      val res =
        post("http://localhost:8080/goblin/measure", s"""{"name":"$name"}""")
      if res.statusCode() == 200 then
        val g = read[GoblinResponse](res.body())
        Right(Goblin(g.name, g.weightKg, g.heightCm))
      else Left(Exception(s"Weights & Measures failed: ${res.statusCode()}"))
  ...
  def acquireWeapon(goblin: Goblin): Either[Throwable, Weapon] =
    try
      val res = post(
        "http://localhost:8081/weapon/acquire",
        s"""{"name":"${goblin.name}","weightKg":${goblin.weightKg}}"""
      )
      if res.statusCode() == 200 then
        val w = read[WeaponResponse](res.body())
        Right(Weapon(w.kind, w.size))
      else Left(OutOfStockException("Smithy"))
    catch case e: Throwable => Left(e)

  def returnWeapon(goblin: Goblin): Either[Throwable, Unit] =
    try
      val res = post(
        "http://localhost:8081/weapon/return",
        s"""{"name":"${goblin.name}"}"""
      )
  ...
  def acquireUniform(goblin: Goblin): Either[Throwable, Uniform] =
    try
      val res = post(
        "http://localhost:8082/uniform/acquire",
        s"""{"name":"${goblin.name}","heightCm":${goblin.heightCm}}"""
      )
      if res.statusCode() == 200 then
        val u = read[UniformResponse](res.body())
        Right(Uniform(u.size, u.color))
      else Left(OutOfStockException("Rags & Style"))
    catch case e: Throwable => Left(e)

  def returnUniform(goblin: Goblin): Either[Throwable, Unit] =
    try
      val res = post(
        "http://localhost:8082/uniform/return",
        s"""{"name":"${goblin.name}"}"""
      )
      if res.statusCode() == 200 then Right(())
  ...
  def acquireBoots(goblin: Goblin): Either[Throwable, Boot] =
    try
      val res = post(
        "http://localhost:8083/boots/acquire",
        s"""{"name":"${goblin.name}","weightKg":${goblin.weightKg}}"""
      )
      if res.statusCode() == 200 then
        val b = read[BootsResponse](res.body())
        Right(Boot(b.size))
      else Left(OutOfStockException("Cobblery"))
    catch case e: Throwable => Left(e)

  def returnBoots(goblin: Goblin): Either[Throwable, Unit] =
    try
      val res = post(
        "http://localhost:8083/boots/return",
        s"""{"name":"${goblin.name}"}"""
      )
      if res.statusCode() == 200 then Right(())
  ...
  def sendPortrait(goblin: Goblin): Either[Throwable, Unit] =
    try
      val res = post(
        "http://localhost:8084/portrait/send",
        s"""{"name":"${goblin.name}"}"""
      )
      if res.statusCode() == 200 then Right(())
