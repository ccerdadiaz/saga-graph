...
object SmithyService:
  ...
  def getAvailable(): List[String] =
    synchronized { scala.util.Random.shuffle(catalog.filter(_.available).map(_.id).toList) }
  ...
  // Raw acquire without random latency — for demos that need controlled timing
  def acquireRaw(weaponId: String): Either[Throwable, Weapon] =
    synchronized:
      catalog.find(w => w.id == weaponId && w.available) match
        case Some(w) =>
          w.available = false
          log.info(s"[Smithy] $weaponId acquired (raw). Available: ${getAvailable().mkString(", ")}.")
          Right(Weapon(w.id, w.label))
        case None =>
          Left(OutOfStockException("Smithy"))
  ...
  def resetWithStock(n: Int): Unit = synchronized:
    catalog.clear()
    (1 to n).foreach(i => catalog += WeaponEntry(s"sword-$i",
      if i % 2 == 0 then "standard short sword" else "heavy short sword", available = true))
...
object RagsAndStyleService:
  ...
  def getAvailable(size: String): List[String] =
    synchronized { scala.util.Random.shuffle(catalog.filter(u => u.available && u.size == size).map(_.id).toList) }
  ...
  def resetWithStock(n: Int): Unit = synchronized:
    ...
    (1 to n).foreach(i => catalog += UniformEntry(s"uniform-$i", sizes((i-1) % 4), available = true))
...
object CobbleryService:
  ...
  def getAvailable(size: Int): List[String] =
    synchronized { scala.util.Random.shuffle(catalog.filter(b => b.available && b.bootSize == size).map(_.id).toList) }
  ...
  def resetWithStock(n: Int): Unit = synchronized:
    ...
    (1 to n).foreach(i => catalog += BootsEntry(s"boots-$i", sizes((i-1) % 4), available = true))
